 ...
 *
 * (wrapped up in a function so that the `sailsAppPath` can be passed down into it)
 *
 ...
 * (currently there's no way to change the require path in consolidate)
 */

 ////////////////////////////////////////////////////////////////////////////////
// TODO:
// Examine using this approach instead (that we were originally going to use for multipart)
// It's a good fit since consolidate doesn't have any dependencies.
// And it would allow us to keep up to date w/ the latest version of consolidate.
// Of course the best thing would be for a PR to consolidate that allows us to configure the
// require path to be accepted...  :/
//
//
// eval and return the configured middleware
// (This is trusted code, and it's only run once-- on lift.)
// try {
//  eval('multipartBodyParser = '+ (function () {
//    var console = { warn: function () {} };
//    // .. Problem: other closure variables, including private connect modules,
//    // multiparty dependency, and qs dependency would need to be set for this to work...
//    (MULTIPART)(options);
//  }).toString().replace('MULTIPART', express.multipart.toString()));
// }
// catch (e) {
//  sails.log.error('Unexpected error using Connect HTTP multipart body parser:');
//  sails.log.error('(something probably changed in Express or Connect core...)');
//  sails.log.error(e);
// }
////////////////////////////////////////////////////////////////////////////////
...
module.exports = function(sailsAppPath) {
    ...
    if (!engine) {
      try {
        ...
        try {
          ...
          throw new Error('Could not load Jade module; please do "npm install --save jade"');
        }
      }
    }
  ...
  fns.ractive.render = function(str, options, fn){
    var layout = sails.config.views.layout;
    if (Object.keys(options).indexOf('layout') > -1)
      layout = options.layout;

    if (!layout)
      return ractiveRender('{{> body }}', str, options, fn);

    var file = layout + '.' + sails.config.views.engine.ext;
    var path = sails.config.paths.views + '/' + file;
    // only use cache on production
    var cache = sails.config.environment === 'production';
    read(path, {cache: cache}, function (err, layout) {
      if (err)
        return fn(err);
      ractiveRender(layout, str, options, fn);
    });
  };

  var ractiveRender = function(layout, body, options, fn) {
  ...
  fns.twig = function(path, options, fn) {
    var engine = requires.twig || (requires.twig = require(sailsAppPath + '/twig'));
    engine.renderFile(path, options, fn);
  };
  ...
  fns.twig.render = function(str, options, fn) {
    var engine = requires.twig || (requires.twig = require(sailsAppPath + '/twig').twig);
    engine.render(str, options, fn);
  };
